...
/**
 * This class make a pop up window asking for the username and password fields
 * and two buttons options, "Login" and "Close".
 * If the correct username an password are given when Login button is press the
 * userLogged method in User is call and the user state machine menu/interaction
 * is started.
 * 
...
public class LogUser implements ActionListener {
	...
	protected static void logUser() {
		...
		// Make password text box to enter his/her password and don't display what is
		// input
		password = new JPasswordField();
		password.setBounds(100, 50, 165, 25);
		panel.add(password);

		// Make a Login button which when press it calls ActionPerformed with the
		// ActionEvent having Login
		JButton button = new JButton("Login");
		button.setBounds(100, 80, 70, 20);
		button.addActionListener(new LogUser());
		panel.add(button);

		// Make an Exit button which when press it calls ActionPerformed with the
		// ActionEvent having Exit
